...
## Implementation in pdfdancer-client-typescript-examples

- Added `CLIPPING_TEST_PATH` in `examples/shared.ts` so examples can open `examples/clipping/invisible-content-clipping-test.pdf`, the repo's sample PDF with hidden clipped content.
- Added `examples/working-with-images/11_clear_image_clipping.ts` as the concrete clear-clipping example for this repo. It opens the clipping test PDF, selects images on page 1, calls `image.clearClipping()` on the first image, and saves the revealed result to `output/working-with-images/clear_image_clipping.pdf`.
- Updated `examples/working-with-images/README.md` so the image examples index documents the new script and notes that it uses the clipping-specific sample PDF instead of `examples/Showcase.pdf`.
- Updated `package.json` and `package-lock.json` to pin `pdfdancer-client-typescript` to commit `834b4bc2ab3f2d19b64702d2a668ce2e96762fc1`, which is the SDK revision in this repo that exposes `clearClipping()` for the example script.
- Repo-specific detail: this repository does not add any API endpoint handling itself; it demonstrates the SDK helper against a prepared fixture PDF and writes the modified document to the local `output/` tree.
